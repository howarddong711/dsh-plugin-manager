window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function starCount(plugin) {
			if (plugin.stars === null || plugin.stars === void 0 || plugin.stars === "") return null;
		...
		function pluginCard({ plugin, current, mode, busy, onAction, t }) {
			...
			current ? `${current.enabled ? t("enabled") : t("disabled")}${current.version ?? t("versionUnknown")}` : `${plugin.version ?? t("versionUnknown")}${plugin.repository ?? plugin.id}`;
			return (0, react.createElement)("article", {
				...
				}, t("github")) : (0, react.createElement)("span", { key: "repository" }, plugin.repository ?? plugin.id), (0, react.createElement)("span", { key: "stars" }, stars === null ? t("starsUnknown") : t("stars", { count: stars }))]),
				(0, react.createElement)("div", {
					key: "actions",
					role: "group",
		...
		function ManagerPanel({ mode, t }) {
			...
			const visible = (mode === MARKETPLACE_MODE ? plugins : installed).filter((plugin) => matchesQuery(plugin, query)).sort(mode === MARKETPLACE_MODE ? compareByStars : () => 0);
